...
// <bitbar.title>Azure Devops Pull Requests</bitbar.title>
...
const project = "Engineering";
const user = "jens.fredskov@nekohealth.com";
const query = "[].{desc: description}";
const baseCommand = `az repos pr list --organization "${org}" --project "${project}" --query "${query}"`;

const mine_prs = await $`${baseCommand} --creator "${user}"`.json();
const review_prs = await $`${baseCommand} --reviewer "${user}"`.json();

console.log();

// 󰘧 az repos pr list --creator "jens.fredskov@nekohealth.com" --organization "https://dev.azure.com/cur8/" --project "Engineering" --query "[].{desc: description}"
